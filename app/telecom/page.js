...
function saveSession()  { sessionStorage.setItem(SESSION_KEY, JSON.stringify({ expiry: Date.now() + SESSION_TTL })); }
...
function LoginScreen({ onSuccess }) {
  ...
  const handleLogin = () => {
    ...
    setLoading(true);
    setTimeout(() => {
      const ok = CREDENTIALS.some(c => c.username === user.trim() && c.password === pass);
      if (ok) { saveSession(); onSuccess(); }
      else {
  ...
  return (
    <div
      ...
      style={{
        backgroundImage: `
          radial-gradient(ellipse 80% 60% at 50% -10%, rgba(202,138,4,0.12), transparent),
          radial-gradient(ellipse 60% 40% at 80% 100%, rgba(202,138,4,0.06), transparent)
        `,
      }}
    >
      <div style={{ animation: shake ? 'shake 0.4s ease' : 'none' }} className="w-full max-w-sm">
        {/* Logo */}
        <div className="text-center mb-8">
          <h1 className="text-4xl font-black text-[#ca8a04] tracking-widest mb-1">MO CONTROL</h1>
          <p className="text-[11px] text-gray-600 tracking-[0.3em] uppercase">Telecom Management</p>
          <div className="mt-3 mx-auto w-16 h-px bg-gradient-to-r from-transparent via-[#ca8a04]/50 to-transparent" />
        </div>

        {/* Card */}
        <div className="bg-[#111] border border-gray-800 rounded-3xl p-6 shadow-2xl shadow-black/60">
          <p className="text-center text-xs text-gray-500 mb-6 font-bold">تسجيل الدخول</p>

          {/* Username */}
          <div className="mb-4">
            <label className="text-[10px] text-gray-500 font-bold block mb-1.5 px-1">اسم المستخدم</label>
            <div className="relative">
              <span className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-600 text-sm">👤</span>
              <input type="text" value={user} onChange={e => { setUser(e.target.value); setError(''); }} onKeyDown={handleKey}
                placeholder="ادخل اسم المستخدم" autoComplete="username"
                className="w-full bg-black border border-gray-800 rounded-xl py-3 pr-9 pl-4 text-sm text-white outline-none focus:border-[#ca8a04] transition-colors placeholder-gray-700" />
            </div>
          </div>

          {/* Password */}
          <div className="mb-5">
            <label className="text-[10px] text-gray-500 font-bold block mb-1.5 px-1">كلمة المرور</label>
            <div className="relative">
              <span className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-600 text-sm">🔒</span>
              <input type={show ? 'text' : 'password'} value={pass} onChange={e => { setPass(e.target.value); setError(''); }} onKeyDown={handleKey}
                placeholder="ادخل كلمة المرور" autoComplete="current-password"
                className="w-full bg-black border border-gray-800 rounded-xl py-3 pr-9 pl-10 text-sm text-white outline-none focus:border-[#ca8a04] transition-colors placeholder-gray-700" />
              <button type="button" onClick={() => setShow(v => !v)}
                className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-600 hover:text-gray-400 text-sm transition-colors">
                {show ? '🙈' : '👁️'}
              </button>
            </div>
          </div>
          ...
          {error && (
            <div className="mb-4 bg-red-900/20 border border-red-900 rounded-xl px-4 py-2.5 text-center">
          ...
          <button onClick={handleLogin} disabled={loading || !user || !pass}
            className={`w-full py-3.5 rounded-xl font-black text-sm transition-all duration-200
              ${loading || !user || !pass
                ...
                : 'bg-[#ca8a04] text-black hover:bg-[#d97706] hover:scale-[1.02] active:scale-[0.98] shadow-lg shadow-[#ca8a04]/20'}`}>
            {loading
              ? <span className="flex items-center justify-center gap-2"><span className="inline-block w-4 h-4 border-2 border-black/30 border-t-black rounded-full animate-spin" />جاري التحقق...</span>
              : 'دخول →'}
          </button>
        </div>

        <p className="text-center text-[10px] text-gray-700 mt-5">🔐 محمي — غير مصرح بالدخول بدون إذن</p>
      </div>
    ...
    <div className="flex flex-col items-center gap-1.5">
      <div className="flex gap-1 items-center">
        {Array.from({ length: total }).map((_, i) => (
          <div key={i} className={`rounded-full transition-all ${i < usedSeats ? 'w-3 h-3 bg-green-500 shadow-[0_0_6px_rgba(74,222,128,0.6)]' : 'w-2.5 h-2.5 bg-gray-700'}`} />
        ))}
      </div>
...
function buildFormattedExcel(allLines) {
  ...
  const COLORS = {
    headerBg: 'FF1565C0', headerFg: 'FFFFFFFF',
    colHdrBg: 'FF1976D2', colHdrFg: 'FFFFFFFF',
    paidBg:   'FFE8F5E9', paidFg:   'FF1B5E20',
    unpaidBg: 'FFFFEBEE', unpaidFg: 'FFB71C1C',
    partialBg:'FFFFF8E1', partialFg:'FFE65100',
    totalBg:  'FF0D47A1', totalFg:  'FFFFFFFF',
    lineBg:   'FFE3F2FD', emptyBg:  'FFF5F5F5',
    altBg:    'FFFAFAFA', white:    'FFFFFFFF',
    doneBg:   'FFE8F5E9', doneFg:   'FF1B5E20',
    notDoneBg:'FFFFEBEE', notDoneFg:'FFB71C1C',
  };
  ...
  Object.entries(groups).forEach(([sheetName, lines]) => {
    const aoa = [], rowStyles = [], merges = [], rowHeights = [];

    const pushRow = (cells, styles, height = 20) => {
      aoa.push(cells); rowStyles.push(styles); rowHeights.push(height);
    };
    ...
    lines.forEach(line => {
      ...
      pushRow(
        [ownerDisplay,'','','','',`جيجا: ${totalGB} GB`,`دقائق: ${totalMins}`,`ج.م. ${baseCost.toLocaleString('ar-EG')}`,`ج.م. ${plSign}${profitLoss.toLocaleString('ar-EG')}`,isH4G?(h.linePhone||''):(line.masterPhone||''),line.billPaid?'✓ مدفوعة':'✗ غير مدفوعة',line.voucherSent?'✓ فواتشر':'✗ فواتشر',line.todSent?'✓ TOD':'✗ TOD',line.activationDate||(isH4G?(h.package||''):'')],
        [cellStyle(COLORS.headerBg,COLORS.headerFg,true,14,'right'),cellStyle(COLORS.headerBg,COLORS.headerFg,false,10),cellStyle(COLORS.headerBg,COLORS.headerFg,false,10),cellStyle(COLORS.headerBg,COLORS.headerFg,false,10),cellStyle(COLORS.headerBg,COLORS.headerFg,false,10),cellStyle(COLORS.headerBg,COLORS.headerFg,true,10),cellStyle(COLORS.headerBg,COLORS.headerFg,true,10),cellStyle(COLORS.headerBg,COLORS.headerFg,true,11),cellStyle(COLORS.headerBg,profitLoss>=0?'FF81C784':'FFEF9A9A',true,12),cellStyle(COLORS.headerBg,COLORS.headerFg,true,10),cellStyle(line.billPaid?COLORS.doneBg:COLORS.notDoneBg,line.billPaid?COLORS.doneFg:COLORS.notDoneFg,true,10),cellStyle(line.voucherSent?COLORS.doneBg:COLORS.notDoneBg,line.voucherSent?COLORS.doneFg:COLORS.notDoneFg,true,10),cellStyle(line.todSent?COLORS.doneBg:COLORS.notDoneBg,line.todSent?COLORS.doneFg:COLORS.notDoneFg,true,10),cellStyle(COLORS.headerBg,'FFFFD54F',true,11)],
        28
      );
      ...
      pushRow(
        ...
        Array(14).fill(cellStyle(COLORS.colHdrBg, COLORS.colHdrFg, true, 9)), 22
      );
      ...
      if (!isH4G) {
        subs.forEach((sub, si) => {
          ...
          const baseStyle = (bold=false,sz=10,hAlign='center') => cellStyle(rowBg,'FF212121',bold,sz,hAlign);

          let payStyle, payVal;
          if (isEmpty)       { payStyle=cellStyle(COLORS.emptyBg,'FFBDBDBD',false,9); payVal=''; }
          else if (isPaid)   { payStyle=cellStyle(COLORS.paidBg,COLORS.paidFg,true,10); payVal='دفع ✓'; }
          else if (isPartial){ payStyle=cellStyle(COLORS.partialBg,COLORS.partialFg,true,10); payVal=`جزئي ${paid}`; }
          else if (isFullDebt){ payStyle=cellStyle(COLORS.unpaidBg,COLORS.unpaidFg,true,10); payVal='لا ✗'; }
          else               { payStyle=baseStyle(); payVal=''; }

          pushRow(
            [sub.gb||'',sub.mins||'',price||'',payVal,sub.name||'',totalMB||'',sub.mins||'',price||'',remainMB||'','',sub.phone||'','','',''],
            [baseStyle(),baseStyle(),baseStyle(),payStyle,baseStyle(true,11,'right'),baseStyle(false,9),baseStyle(false,9),baseStyle(false,9),cellStyle(rowBg,remainMB<0?'FFEF5350':'FF43A047',false,9),baseStyle(false,9),baseStyle(false,10,'center'),baseStyle(false,9),baseStyle(false,9),baseStyle(false,9)],
            20
          );
        });
      } else {
        ...
        pushRow(
          [h.package||'','',hCost||'',hStatus,h.subscriberName||'','','',hPaid||'',hDebt>0?`-${hDebt}`:'✓',h.linePhone||'',h.ownerName||'',h.discountPhone||'',h.contactPhone||'','🏠'],
          [cellStyle(COLORS.lineBg,'FF212121',false,10),cellStyle(COLORS.lineBg,'FF212121',false,10),cellStyle(COLORS.lineBg,'FF212121',false,10),cellStyle(statusBg,statusFg,true,10),cellStyle(COLORS.lineBg,'FF212121',true,11,'right'),cellStyle(COLORS.lineBg,'FF212121',false,10),cellStyle(COLORS.lineBg,'FF212121',false,10),cellStyle(COLORS.paidBg,COLORS.paidFg,true,10),cellStyle(hDebt>0?COLORS.unpaidBg:COLORS.paidBg,hDebt>0?COLORS.unpaidFg:COLORS.paidFg,true,10),cellStyle(COLORS.lineBg,'FF212121',false,10),cellStyle(COLORS.lineBg,'FF212121',false,10),cellStyle(COLORS.lineBg,'FF212121',false,10),cellStyle(COLORS.lineBg,'FF212121',false,10),cellStyle(COLORS.lineBg,'FFCA8A04',true,14)],
          22
        );
      }

      const totalPaidCalc = isH4G?Number(h.paidAmount||0):subs.reduce((a,s)=>a+Number(s.paidAmount||0),0);
      pushRow(
        ['الإجمالي','','','',`${isH4G?1:subs.filter(s=>s.name||s.phone).length} مشترك`,`${isH4G?0:usedGB} GB`,isH4G?'':`${usedMins}`,`ج.م. ${totalPrice.toLocaleString('ar-EG')}`,`ج.م. ${plSign}${profitLoss.toLocaleString('ar-EG')}`,'','','','',''],
        [thickCellStyle(COLORS.totalBg),thickCellStyle(COLORS.totalBg),thickCellStyle(COLORS.totalBg),thickCellStyle(COLORS.totalBg),thickCellStyle(COLORS.totalBg,COLORS.totalFg,true,10),thickCellStyle(COLORS.totalBg,COLORS.totalFg,true,10),thickCellStyle(COLORS.totalBg,COLORS.totalFg,false,10),thickCellStyle(COLORS.totalBg,COLORS.totalFg,true,11),thickCellStyle(COLORS.totalBg,profitLoss>=0?'FF81C784':'FFEF9A9A',true,12),...Array(5).fill(thickCellStyle(COLORS.totalBg))],
        24
      );
...
// Main Component
// ─────────────────────────────────────────────────────────────────
function TelecomSystem({ onLogout }) {
  ...
  const subscribeToLines = useCallback(() => {
    ...
    const q = query(collection(db,'lines'), where('network','==',activeTab), where('cycle','==',activeCycle));
  ...
  const subscribeToAbroad = useCallback(() => {
    ...
    if (isHome4G) return () => {};
    const q = query(collection(db,'abroadClients'), where('network','==',activeTab), where('cycle','==',abroadCycle));
    const unsub = onSnapshot(q, (snap) => {
  ...
  useEffect(() => { const u=subscribeToLines(); return ()=>u(); }, [subscribeToLines]);
  useEffect(() => { const u=subscribeToAbroad(); return ()=>u(); }, [subscribeToAbroad]);

  const handleManualRefresh = useCallback(() => { setIsRefreshing(true); subscribeToLines(); }, [subscribeToLines]);
  ...
  const addNewAbroadClient = useCallback(async () => {
    try {
      await addDoc(collection(db,'abroadClients'), { network: activeTab, cycle: abroadCycle, name: 'عميل جديد', phone: '', gb: 0, mins: 1500, price: 0, paidAmount: 0, traderName: '', activationDate: '' });
    } catch { alert('خطأ في الاتصال بقاعدة البيانات'); }
  ...
  const deleteAbroadClient = useCallback(async (e, id) => {
    e.stopPropagation();
    if (window.confirm('هل تريد حذف هذا العميل نهائياً؟')) await deleteDoc(doc(db,'abroadClients',id));
  }, []);
  ...
  const importFromExcel = useCallback((e) => {
    ...
    reader.onload = async (evt) => {
      ...
      const sName= wb.SheetNames.includes('Backup') ? 'Backup' : wb.SheetNames[wb.SheetNames.length-1];
  ...
  const globalStats = (() => {
    ...
    masterLines.forEach(line => {
      ...
      if (line.network==='Home4G') totalDebt+=s.debts;
      else if (!line.billPaid) totalDebt+=Number(line.baseCost||0);
    });
  ...
  const allSubscribers = (() => {
    ...
    masterLines.forEach(line=>{
      (line.subscribers||[]).forEach((sub,idx)=>{
        ...
        result.push({ lineId:line.id, lineOwner:line.ownerName||'بدون اسم', masterPhone:line.masterPhone||'', subIndex:idx, name:sub.name||'', phone:sub.phone||'', gb:sub.gb||0, price:sub.price||0, paidAmount:sub.paidAmount||0, debt:Number(sub.price||0)-Number(sub.paidAmount||0), subscribers:line.subscribers });
      });
    });
    return result;
  })();
  ...
  const subsStats    = { totalDebt: allSubscribers.filter(s=>s.debt>0).reduce((a,s)=>a+Math.max(0,s.debt),0), totalPaid: allSubscribers.filter(s=>s.debt>0).reduce((a,s)=>a+s.paidAmount,0), totalPrice: allSubscribers.filter(s=>s.debt>0).reduce((a,s)=>a+s.price,0) };
  ...
  const filteredSubs = allSubscribers.filter(sub => {
    const matchSearch = !searchTerm||(()=>{ const s=searchTerm.toLowerCase(); return sub.name?.toLowerCase().includes(s)||sub.phone?.includes(searchTerm)||sub.lineOwner?.toLowerCase().includes(s); })();
    const isFullDebt=sub.paidAmount===0&&sub.debt>0; const isPartial=sub.paidAmount>0&&sub.debt>0; const hasDebt=sub.debt>0;
  ...
  const filteredSubsStats = { totalPrice:filteredSubs.reduce((a,s)=>a+s.price,0), totalPaid:filteredSubs.reduce((a,s)=>a+s.paidAmount,0), totalDebt:filteredSubs.reduce((a,s)=>a+Math.max(0,s.debt),0) };
  ...
  return (
    <div className="px-3 py-4 sm:px-6 sm:py-6 md:px-8 md:py-8 bg-[#0a0a0a] min-h-screen text-gray-200" dir="rtl">
      ...
      <button onClick={onLogout} title="تسجيل الخروج"
        className="fixed top-3 left-3 z-50 bg-[#111] border border-gray-800 hover:border-red-800 text-gray-600 hover:text-red-400 text-[10px] font-bold px-3 py-1.5 rounded-xl transition-all">
        خروج 🚪
      </button>
      ...
      <div className="flex justify-center gap-2 mb-6 sm:mb-10 flex-wrap">
        <button onClick={() => { setAbroadMode(false); setActiveCycle('1'); setExpandedLine(null); setSubsFilter('all'); }}
        ...
        <button onClick={() => { setAbroadMode(false); setActiveCycle('15'); setExpandedLine(null); setSubsFilter('all'); }}
        ...
        {!isHome4G && (
          <button onClick={() => { setAbroadMode(true); setAbroadCycle('1'); setExpandedAbroad(null); }}
        ...
        {!isHome4G && (
          <button onClick={() => { setAbroadMode(true); setAbroadCycle('15'); setExpandedAbroad(null); }}
        ...
        <div className="max-w-7xl mx-auto">
          <div className="mb-4 flex items-center justify-between bg-[#111] border border-gray-800 rounded-2xl px-4 sm:px-5 py-3">
            <div className="flex items-center gap-2 sm:gap-3">
              ...
              <div>
                <p className={`font-black text-xs sm:text-sm ${netMeta?.color}`}>عملاء الخارج — {netMeta?.label} سايكل {abroadCycle}</p>
            ...
            {filteredAbroad.map((client) => {
              ...
              return (
                <div key={client.id} className="bg-[#111] border border-gray-800 rounded-2xl sm:rounded-3xl overflow-hidden shadow-xl">
                  <div onClick={()=>setExpandedAbroad(isOpen?null:client.id)} className="p-3 sm:p-4 cursor-pointer hover:bg-[#161616] transition-colors">
                    <div className="flex items-start justify-between gap-2 mb-3">
                      <div className="flex-1 min-w-0">
                        <p className="font-bold text-white text-sm truncate">{client.name||'بدون اسم'} — {client.phone||'0000'}</p>
                        <p className="text-[10px] text-gray-400 mt-0.5 truncate">التاجر: <span className="text-[#ca8a04] font-bold">{client.traderName||'غير محدد'}</span>{client.activationDate?<span className="mr-2 text-gray-500">· {client.activationDate}</span>:null}</p>
                      </div>
                    ...
                    <div className="flex flex-wrap gap-1.5 sm:gap-2 items-center">
                      ...
                      <div className={`p-2 rounded-lg border min-w-[70px] text-center ${isPaid?'border-green-700 bg-green-900/20':'border-red-900 bg-red-900/20'}`}>
                        ...
                        <p className={`font-bold text-xs ${isPaid?'text-green-400':isPartialPay?'text-yellow-400':'text-red-400'}`}>{isPaid?'خالص ✓':`${debt} ج`}</p>
                      </div>
                      <button onClick={(e)=>{e.stopPropagation();handleClearAbroadDebt(client.id,client.price);}}
                    ...
                    <div className="p-4 sm:p-6 border-t border-gray-800 bg-[#0d0d0d]">
                      ...
                      <div className="overflow-x-auto -mx-4 px-4 sm:mx-0 sm:px-0">
                        <div className="grid grid-cols-4 sm:grid-cols-8 gap-2 sm:gap-3 items-end bg-[#111] p-3 sm:p-4 rounded-2xl border border-gray-800 min-w-[480px] sm:min-w-[900px]">
                          {[
                            {label:'الاسم',field:'name',cls:'text-white',type:'text'},
                            {label:'الرقم',field:'phone',cls:'text-white',type:'text'},
                            {label:'اسم التاجر',field:'traderName',cls:'text-[#ca8a04]',type:'text'},
                          ].map(({label,field,cls,type})=>(
                            <div key={field} className="flex flex-col gap-1">
                              <label className="text-[9px] text-gray-500 text-center">{label}</label>
                              <input type={type} defaultValue={client[field]} onChange={(e)=>updateAbroadClient(client.id,field,e.target.value,client)}
                          ...
                          <div className="flex flex-col gap-1">
                            ...
                            <input defaultValue={client.activationDate||''} onChange={(e)=>updateAbroadClient(client.id,'activationDate',e.target.value,client)} placeholder="1/4"
                              className="bg-black border border-gray-800 rounded-lg p-2 text-[11px] text-[#ca8a04] outline-none focus:border-[#ca8a04] text-center w-full" />
                          </div>
                          {[
                            {label:'الدقائق',field:'mins',cls:'text-white'},
                            {label:'السعر',field:'price',cls:'text-white'},
                            {label:'المدفوع',field:'paidAmount',cls:'text-green-400'},
                          ].map(({label,field,cls})=>(
                            <div key={field} className="flex flex-col gap-1">
                              <label className="text-[9px] text-gray-500 text-center">{label}</label>
                              <input type="number" defaultValue={client[field]} onChange={(e)=>updateAbroadClient(client.id,field,e.target.value,client)}
                                className={`bg-black border border-gray-800 rounded-lg p-2 text-[11px] ${cls} outline-none focus:border-[#ca8a04] text-center w-full`} />
                            </div>
                          ))}
                        </div>
          ...
          {filteredLines.map(line => {
            ...
            return (
              <div key={line.id} className="bg-[#111] border border-gray-800 rounded-2xl sm:rounded-3xl overflow-hidden shadow-xl">
                <div onClick={()=>setExpandedLine(isOpen?null:line.id)} className="p-3 sm:p-4 cursor-pointer hover:bg-[#161616] transition-colors">
                ...
                {isOpen && !isH4G && (
                  <div className="p-3 sm:p-6 border-t border-gray-800 bg-[#0d0d0d]">
                    <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-7 gap-2 sm:gap-3 md:gap-4 mb-5 sm:mb-8 bg-[#161616] p-3 sm:p-4 rounded-xl sm:rounded-2xl border border-gray-800">
                      {[
                        {l:'الاسم',k:'ownerName',t:'text'},{l:'الرقم',k:'masterPhone',t:'text'},
                        {l:'تاريخ التفعيل',k:'activationDate',t:'text'},{l:'إجمالي الجيجا',k:'totalGB',t:'number'},
                        {l:'إجمالي الدقائق',k:'totalMins',t:'number'},{l:'التكلفة',k:'baseCost',t:'number'},
                      ].map(({l,k,t})=>(
                        ...
                        return (
                          <div key={index}
                            className={`grid gap-1.5 sm:gap-2 items-center p-2 sm:p-3 rounded-xl sm:rounded-2xl border transition-all min-w-[700px] sm:min-w-[900px]
                              ${isFilled?'bg-[#111] border-gray-700 hover:border-gray-600':'bg-[#0d0d0d] border-dashed border-gray-800 opacity-60 hover:opacity-80'}`}
                            style={{gridTemplateColumns:'auto 1fr 1fr 1fr 1fr 1fr 1fr 1fr 1fr auto'}}>
                            <div className={`w-6 h-6 rounded-full flex items-center justify-center text-[9px] font-black flex-shrink-0 ${isFilled?'bg-green-900/60 text-green-400 border border-green-700':'bg-gray-900 text-gray-600 border border-gray-800'}`}>{index+1}</div>

                            <SubField label="الاسم"><input defaultValue={sub.name} onChange={(e)=>updateSub(line.id,index,'name',e.target.value,line.subscribers,line.network)} className={`bg-black border border-gray-800 rounded-lg p-1.5 sm:p-2 text-[11px] sm:text-[12px] outline-none w-full ${isFilled?'text-white focus:border-[#ca8a04]':'text-gray-600 focus:border-gray-600 placeholder-gray-700'}`} placeholder="الاسم" /></SubField>
                            <SubField label="الرقم"><input defaultValue={sub.phone} onChange={(e)=>updateSub(line.id,index,'phone',e.target.value,line.subscribers,line.network)} className="bg-black border border-gray-800 rounded-lg p-1.5 sm:p-2 text-[11px] sm:text-[12px] text-white outline-none focus:border-[#ca8a04] w-full" placeholder="الرقم" /></SubField>
                            <SubField label="باقة GB"><GbInput defaultValue={sub.gb||''} network={line.network} listId={`gb-list-${line.id}-${index}`} onChange={(e)=>updateSub(line.id,index,'gb',e.target.value,line.subscribers,line.network)} /></SubField>
                            <SubField label="الدقائق"><input type="number" defaultValue={sub.mins} onChange={(e)=>updateSub(line.id,index,'mins',e.target.value,line.subscribers,line.network)} className="bg-black border border-gray-800 rounded-lg p-1.5 sm:p-2 text-[11px] sm:text-[12px] text-white outline-none focus:border-[#ca8a04] w-full" /></SubField>
                            <SubField label="مُرسل MB"><input type="number" defaultValue={sub.sentMB} onChange={(e)=>updateSub(line.id,index,'sentMB',e.target.value,line.subscribers,line.network)} className="bg-black border border-gray-800 rounded-lg p-1.5 sm:p-2 text-[11px] sm:text-[12px] text-white outline-none focus:border-[#ca8a04] w-full" /></SubField>
                            <SubField label="متبقي MB"><span className={`text-[11px] sm:text-[12px] font-bold p-1.5 sm:p-2 block text-center rounded-lg ${remainMB<0?'text-red-500 bg-red-900/10':'text-green-500 bg-green-900/10'}`}>{remainMB}</span></SubField>
                            <SubField label="السعر"><input type="number" defaultValue={sub.price} onChange={(e)=>updateSub(line.id,index,'price',e.target.value,line.subscribers,line.network)} className="bg-black border border-gray-800 rounded-lg p-1.5 sm:p-2 text-[11px] sm:text-[12px] text-white outline-none focus:border-[#ca8a04] w-full" /></SubField>
                            <SubField label="المدفوع"><input type="number" defaultValue={sub.paidAmount} onChange={(e)=>updateSub(line.id,index,'paidAmount',e.target.value,line.subscribers,line.network)} className="bg-black border border-gray-800 rounded-lg p-1.5 sm:p-2 text-[11px] sm:text-[12px] text-white outline-none focus:border-[#ca8a04] w-full" /></SubField>
                          ...
                          <>
                            ...
                            <div className="flex gap-1">
                              {Array.from({length:MAX_SUBS}).map((_,i)=>{
                                ...
                                return <div key={i} className={`rounded-full transition-all ${filled?'w-2.5 h-2.5 bg-green-500':'w-2 h-2 bg-gray-700'}`} />;
                              })}
                            </div>
                          </>
                        );
      ...
      {!isHome4G && !abroadMode && (
        <div className="max-w-7xl mx-auto mt-8 sm:mt-10">
          <button onClick={()=>setShowSubsList(v=>!v)}
            ...
            <div className="flex items-center gap-2 sm:gap-3">
              ...
              <div className="text-right">
                <p className={`font-black text-xs sm:text-base ${netMeta?.color}`}>العملاء اللي عليهم فلوس — {netMeta?.label} سايكل {activeCycle}</p>
              ...
              <div className="bg-[#111] border border-gray-800 rounded-b-2xl overflow-hidden">
                {filteredSubs.length===0 ? (
                  <div className="text-center text-gray-600 py-12 sm:py-16 text-sm">
                    {searchTerm?'لا توجد نتائج للبحث':subsFilter==='full'?'لا يوجد عملاء عليهم دين كامل 🎉':subsFilter==='partial'?'لا يوجد عملاء دفعوا جزء من الدين':'لا يوجد عملاء عليهم ديون 🎉'}
                  </div>
                ) : (
                        ...
                        {filteredSubs.map((sub,i)=>{
                          ...
                          return (
                            <tr key={`${sub.lineId}-${sub.subIndex}`}
                              ...
                              <td className="px-2 sm:px-3 py-2 sm:py-2.5 text-center"><span className={`text-[10px] font-bold px-1.5 py-1 rounded-lg ${netMeta?.bg} ${netMeta?.color} border ${netMeta?.border}`}>{sub.lineOwner}</span></td>
                              ...
                              <td className="px-2 sm:px-3 py-2 sm:py-2.5 text-center text-[11px]">{sub.paidAmount>0?<span className="text-yellow-400 font-bold">{sub.paidAmount} ج</span>:<span className="text-gray-700">—</span>}</td>
                              <td className="px-2 sm:px-3 py-2 sm:py-2.5 text-center"><span className={`font-black text-[12px] ${isPartialPay?'text-yellow-400':'text-red-400'}`}>{sub.debt} ج</span></td>
      ...
      <div className="max-w-7xl mx-auto mt-8 sm:mt-10 pb-6 sm:pb-8 flex justify-end gap-2 sm:gap-3">
        ...
        <button onClick={abroadMode?addNewAbroadClient:addNewLine} title={abroadMode?'إضافة عميل خارج جديد':'إضافة خط جديد'}
...
function GbInput({ defaultValue, network, onChange, listId }) {
  ...
  return (
    <div className="relative w-full">
      <input list={listId} defaultValue={defaultValue||''} onChange={onChange} placeholder="GB"
      ...
      <datalist id={listId}>
        {knownGbs.map(g=><option key={g} value={g}>{g} GB — {PRICE_TABLE[network][g]} ج</option>)}
      </datalist>
    </div>
  );
}
...
export default function App() {
  ...
  return <TelecomSystem onLogout={handleLogout} />;
}
