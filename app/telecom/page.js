                   ...
                   {[...Array(7)].map((_, index) => {
                      ...
                      return (
                        <div key={index} className="grid grid-cols-2 md:grid-cols-10 gap-2 items-center bg-[#111] p-3 rounded-2xl border border-gray-800 mb-2 text-center">
                          <div className="flex flex-col gap-1"><label className="text-[10px] font-bold text-gray-400">الاسم</label><input value={sub.name} onChange={(e) => updateSub(line.id, index, 'name', e.target.value, line.subscribers)} className="bg-black border border-gray-800 rounded-lg p-2 text-[12px] text-white"/></div>
                          <div className="flex flex-col gap-1"><label className="text-[10px] font-bold text-gray-400">الرقم</label><input value={sub.phone} onChange={(e) => updateSub(line.id, index, 'phone', e.target.value, line.subscribers)} className="bg-black border border-gray-800 rounded-lg p-2 text-[12px] text-white"/></div>
                          <div className="flex flex-col gap-1"><label className="text-[10px] font-bold text-gray-400">الباقة (GB)</label><select value={sub.gb} onChange={(e) => updateSub(line.id, index, 'gb', Number(e.target.value), line.subscribers)} className="bg-black border border-gray-800 rounded-lg p-2 text-[12px] text-blue-400"><option value="0">0</option>{Object.keys(priceTable[line.network]).map(g => <option key={g} value={g}>{g}</option>)}</select></div>
                          <div className="flex flex-col gap-1"><label className="text-[10px] font-bold text-gray-400">الدقائق</label><input type="number" value={sub.mins} onChange={(e) => updateSub(line.id, index, 'mins', Number(e.target.value), line.subscribers)} className="bg-black border border-gray-800 rounded-lg p-2 text-[12px] text-white"/></div>
                          ...
                          <div className="flex flex-col gap-1"><label className="text-[10px] font-bold text-gray-400">المُرسل (MB)</label><input type="number" value={sub.sentMB} onChange={(e) => updateSub(line.id, index, 'sentMB', Number(e.target.value), line.subscribers)} className="bg-black border border-gray-800 rounded-lg p-2 text-[12px] text-white"/></div>
                          ...
                          <div className="flex flex-col gap-1"><label className="text-[10px] font-bold text-gray-400">السعر</label><input type="number" value={sub.price} onChange={(e) => updateSub(line.id, index, 'price', Number(e.target.value), line.subscribers)} className="bg-black border border-gray-800 rounded-lg p-2 text-[12px] text-white"/></div>
                          <div className="flex flex-col gap-1"><label className="text-[10px] font-bold text-gray-400">المدفوع</label><input type="number" value={sub.paidAmount} onChange={(e) => updateSub(line.id, index, 'paidAmount', Number(e.target.value), line.subscribers)} className="bg-black border border-gray-800 rounded-lg p-2 text-[12px] text-white"/></div>
                          <button onClick={() => updateSub(line.id, index, 'paidAmount', sub.price, line.subscribers)} className={`text-[10px] font-bold mt-4 ${debt > 0 ? 'text-red-500' : 'text-green-500'}`}>{debt > 0 ? `باقي ${debt}` : 'خالص ✓'}</button>
                        </div>
                      );
                    })}
                </div>
